...
def ler_logs(arquivo):
    ...
    if re.findall("tipo=\d",arquivo,)[0][-1] == '4':
        df_final["Pontuacao_1"] = df[["objetivos"]].explode("objetivos")["objetivos"].apply(lambda x: x[0]*-1).reset_index(drop="True")
        df_final["Pontuacao_2"] = df[["objetivos"]].explode("objetivos")["objetivos"].apply(lambda x: x[1] if len(x)>1 else math.inf).reset_index(drop="True")
    else:
...
def animacao_pareto(execution=1):
...
def gerar_relatorios():
    ...
    for j in range (0,4):
        for i in range (1, 6):
            ...
            if grau == 4:
                ultima = ultima.sort_values(["Pontuacao_1","Pontuacao_2"], ascending=False)
